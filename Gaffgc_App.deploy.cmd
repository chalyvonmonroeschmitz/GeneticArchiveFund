...
set _DeploySetParametersFile=%RootPath%Gaffgc_App.SetParameters.xml
)
...
@rem ---------------------------------------------------------------------------------
if not exist "%RootPath%Gaffgc_App.zip" (
echo "%RootPath%Gaffgc_App.zip" does not exist. 
...
set _MSDeployCommandline="%MSDeployPath%msdeploy.exe" -source:package='%RootPath%Gaffgc_App.zip',EncryptPassword='Encryption password is used only if any deployment setting is marked as secure' -dest:%_Destination%,EncryptPassword='Encryption password is used only if any deployment setting is marked as secure' -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -skip:objectname='dirPath',absolutepath='obj\\Release\\Package\\PackageTmp\\App_Data$' -skip:objectname='dirPath',absolutepath='https://gaffgc\.azurewebsites\.net\\App_Data$'
) else (
set _MSDeployCommandline="%MSDeployPath%msdeploy.exe" -source:package='%RootPath%Gaffgc_App.zip',EncryptPassword='Encryption password is used only if any deployment setting is marked as secure' -dest:%_Destination%,EncryptPassword='Encryption password is used only if any deployment setting is marked as secure' -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -skip:objectname='dirPath',absolutepath='obj\\Release\\Package\\PackageTmp\\App_Data$' -skip:objectname='dirPath',absolutepath='https://gaffgc\.azurewebsites\.net\\App_Data$' -setParamFile:"%_DeploySetParametersFile%"
)
...
:usage
echo =========================================================
if not exist "%RootPath%Gaffgc_App.deploy-readme.txt" (
...
start notepad "%RootPath%Gaffgc_App.deploy-readme.txt"
)
echo =========================================================
goto :eof
